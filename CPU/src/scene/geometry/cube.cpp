...
void Cube::ComputeTBN(const Point3f& P, Normal3f* nor, Vector3f* tan, Vector3f* bit) const
{
    *nor = glm::normalize(transform.invTransT() * GetCubeNormal(P));
    ...
    if(GetCubeNormal(P) == Normal3f(1,0,0) || GetCubeNormal(P) == Normal3f(-1,0,0)) {
        *tan = glm::normalize(glm::vec3(transform.T() * glm::vec4(0,1,0,0)));
    } else if(GetCubeNormal(P) == Normal3f(0,1,0) || GetCubeNormal(P) == Normal3f(0,-1,0)) {
        *tan = glm::normalize(glm::vec3(transform.T() * glm::vec4(0,0,1,0)));
    } else {
        *tan = glm::normalize(glm::vec3(transform.T() * glm::vec4(1,0,0,0)));
    }
